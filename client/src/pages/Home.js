...
const Home = (props) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = await findAnimal("https://api.petfinder.com/v2/animals", searchVal, "1", props.accessToken, addToast);
    if (data) {
      ...
      <div className="row"> */}
        <div className="col-md-9">
          ...
          <div className="row" style={{ width: "100%" }}>
            {filter(renderAnimals, (animal) => animal.photos[0] && animal.photos[0].large).map((animal) => (
              <Card key={animal.id} className="col-md-4">
                ...
                <Card.Footer>
                  ...
                  {user && <Button onClick={() => savePet(animal)}>Save Me!</Button>}
                </Card.Footer>
              </Card>
            ))}
          </div>
